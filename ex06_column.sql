...
-- tblCountry에는 어떤 대륙이 있습니까? > 종류? 그룹?
...
--중복값이 존재하지 않을 때 distict > 아무일도 안 일어난다.
...
-- *** 공용 데이터와 개별 데이터는 메모리 한곳에 넣을 수 없다. > 공용(static), 개별(stack)
...
select distinct gender, weight from tblComedian; -- gender와 weight 를 모두 합한 값 중에 중복인걸 없애줌.
...
select
    last || first as name,
    gender                  -- f,m으로 나오는 것을 남자, 여자로 바꿔달라!
from tblComedian;
