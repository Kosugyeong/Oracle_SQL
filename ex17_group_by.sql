...
select distinct buseo from tblInsa; -- 부서 > 7개
...
select
    ...
    -- name -- 개별값(개인의 데이터) > group by에서는 개별값은 가져올 수 없다. 
from tblInsa
    group by buseo;
...
-- 모든 지역을 수동으로 작성,,, x 11 번 작성;;;
-- 신입 사원 입사 > "강릉" 거주
-- 새로운 데이터가 들어오거나 수정되는 데이터가 있다면 쿼리문을 다시 짜야하는 상황이 생기게 된다...
-- 새로운 데이터 / 수정된 데이터에 아주 취약한 방식!!!!! > 정적인 데이터에는 무난하지만 동적인 데이터에서는 좋지 않은 방식이다. 
select
...
--새로운 데이터 / 수정된 데이터에 아주 강함 > 정적/동적인 데이터 모두에 무난하다. 그룹을 나누는 문제에서는 group by를 쓰는게 맞다!!!
select
    city,
    count(*)
from tblInsa
...
from tblInsa                    --1. 60명의 데이터를 가져온다.
    ...
    group by buseo;             --3. 2번을 통과한 사람들을 대상으로 그룹 형성.
...
--서울 직원들 중 남/녀로 그룹을 나눠서 그룹별 평균 급여가 155만원 이상인 그룹만 남긴 후 각 그룹의 인원수는 몇명인가?
select
    substr(ssn, 8, 1),
    count(*)
from tblInsa
...
--tblInsa
-- 부서별 부서인원수, 부서 내 최고급여, 부서 내 최저급여, 부서 내 평균 급여
select
    buseo,
    count(*) as "부서인원수",
    max(basicpay) as "부서 내 최고급여",
    min(basicpay) as "부서 내 최저급여",
    round(avg(basicpay)) as "부서 내 평균 급여"
from tblInsa
    group by buseo;
...
---------아래와 같은 쿼리문으로 해야 요구사항에 맞는 형태로 나옴!----------
select
    buseo,
    count(*),
