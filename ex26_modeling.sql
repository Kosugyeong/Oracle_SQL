...
/*
    ...
    - 논리 > 관계형 데이터베이스의 기본 설정 > 속성 정의 + 도메인 정의 + 키(PK,FK)... > 제대로 된 ERD  
    - 물리 > 특정 DBMS 결정 + DBMS에 종속적인 나머지 설계 + 물리적 식별자..
    
    
    1. ERD, Entitiy Realationship Diagram
    - 엔티티의 관계를 표현한 그림
    - 엔티티간의 관계를 표현한 그림
    - 데이터베이스 모델링 기법 중 하나
    ...
    - 릴레이션 = 객체 = 레코드 = 엔티티
         a. 학생 정보 관리
            ...
            - 학생(아이디, 학생명, 나이, 주소, 연락처) > 학생: 엔티티, (아이디, 학생명, 나이, 주소, 연락처) : 속성
        b. 강의실 정보 관리
    ...
    ---------------------------1~4 => 논리 다이어그램 > 논리 ERD > 프로젝트한 구조를 다른 사람이 보여달라!! 하면 여기까지의 부분을 보여주면 된다. 
    ...
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하며,
      데이터 종속성을 최소화하기 위해 ERD을 수정하는 작업
    ...
    2. 갱신 이상, Update Anormaly
        - 동일한 데이터가 2개 이상의 테이블에 동시 저장 > 그 중 1개는 수정했지만
            다른 1개를 수정하지 못했을 때 상황 > 두 데이터간의 이상 오류 발생
    ...
    1. 완전 함수 종속, Full Functional Dependency > 기본키가 아닌 컬럼이 기본키에게 종속적인 상태
    ...
    주제 > 네이버 영화
    - 영화 랭킹 안에 들어있는 영화들로만 > 50편 영화
    효율적으로 저장하고 관리하는 측면으로만!!
        
    테이블 개수 10몇개에서 많게는 20~30개
    
    1. 데이터 수집 및 분석
        - 모든 데이터 인식 
        - DB 데이터베이스로 저장할 데이터 선별
...
---------------------------------------------------------- 오늘의 적당한 속도        
    ...
    5. SELECT 작성 ( 위에 부분 진행되는거 보고 천천히...)
