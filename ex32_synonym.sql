...
grant connect, resource to scott; --접속 + 자원 줄 수 있는 권한
...
select * from hr.tblInsa; --다른 계정의 자원은 접근  >그래서 grant로 select와 delete 권한 줬다 

--ORA-01031: insufficient privileges > 권한 없을 때 뜨는 오류
update hr.tblInsa set basicpay = 30000000 where name = '홍길동'; --일반키 > 비인덱스
update hr.tblInsa set basicpay = 30000000 where num = '1001'; --PK > 인덱스 > where절에는 pk가 오는게 속도가 빠르고 pk가 오는게 좋다.!!
...
select * from hr.tblInsa;
select * from insa; -- 소유주 명시x > hr.tabInsa를 insa로 별칭을 줘서 별칭으로 select 가능
