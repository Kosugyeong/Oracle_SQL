...
이 프로그램은 Oracle이 아니라 Oracle에 접속하게 해주는 툴
...
- 수업할 때는 hr계정으로.. > 프로젝트할 때는 계정 직접 생성해서
...
select * from departments; -- 직원들의 부서 
...
오라클 서비스
- 화면 출력X
- 오라클 상태 확인?? 만약 오라클이 제대로 실행 안되면!
    - Window + R(실행) > "services.msc"
    - OracleServiceXE > 오라클 데이터 서버
    - OracleXETNSListener > 오라클이 클라이언트 툴의 접속을 받아들이는 기능
    => 위 두 항목이 '실행 중'이라고 되어있는지 확인하기
...
4. 종류
    a. 표준 SQL(ANSI)
        - 모든 DBMS > 공통 > 사용하는 DBMS가 바뀌어도 공부 안해도됨
    
    b. 각 DBMS 제작사별 SQL > 확장 SQL
        - DBMS 별로 문법 서로 다름 > 사용하는 DBMS가 바뀌면 다시 공부해야함
        - 오라클 > PL/SQL
...
select * from tabs where table_name = 'JOBS'; -- SQL은 데이터의 대/소문자를 구분한다.***)
--select * from tabs where table_name = 'jobs'; -- SQL은 상수의 대/소문자를 구분한다.

-- 1. 키워드 > 대문자
-- 2. 식별자 > 소문자 
-- 이와 같은 스타일을 가장 많이 사용함
SELECT * FROM tabs;
...
/*
    ...
    - 학생(번호, 이름, 나이, 성별) > 릴레이션 스키마 표기
        ...
        - 이름: 2~5이내의 한글
        - 나이: 14~16세 이내의 정수
        - 성별: M/F 1answk xordlf
    - 차수: 4 > 속성(컬럼)의 개수
    - 카디널리티 > 행의 개수
    
    [번호]    [이름]    [나이]    [성별]        [취미]
    1           홍길동     14      M           독서, 먹방보기 > 여러개의 값을 하나의 속성값 안에 넣을 수 없음
    2           아무개     15      F            음악듣기
    3           호호호     16      F            낮잠
    3           호호호     16      F > 지양
