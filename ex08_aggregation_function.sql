...
select max(name), min(name) from tblInsa; --오름차순 정렬했을 때 제일 마지막에 나오는 이름 > 황진이
...
select count(*), name from tblInsa; -- 컬럼 리스트에 집계 함수와 일반 컬럼은 동시에 사용할 수 없다. 
...
select * from tblInsa where basicpay >= avg(basicpay);--  where절에는 집계 함수를 사용할 수 없다.  
