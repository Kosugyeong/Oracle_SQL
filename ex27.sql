...
-- 일정 관리할 때 아래 쿼리가 잘 쓰인다. 
select
    level
from dual
    connect by level <=10; -- connect by는 조건절 같은 것이다(where절 혹은 on 같은 것)


-- 2022-05-01 ~ 2022-05-31 > date형식으로 바꾸는 방법
-- 1. ANSI-SQL > 계층형 쿼리
-- 2. PL/SQL
-- 3. Java
...
select 
    sysdate + (level - 1) 
from dual
    connect by level <= 31; -- level이 결국 루프 횟수를 결정해주는 것이다.
...
delete from tblDate where seq = 15; --15일은 결석! 결석 데이터는 데이터가 남지 않는다. 결석 데이터를 남기려고 하지 말자. 결석 데이터는 그냥 아예 없는 것이다.
...
select
    to_date('2022-03-18', 'yyyy-mm-dd') - to_date('2022-03-01', 'yyyy-mm-dd') --17
from dual;

-- *********** 잘 기억하기 > date 자료형으로 원하는 기간의 레코드를 얻어오는 방법
...
-- 공휴일 처리 > 따로 데이터를 가지고 있어야하며 사람이 일일이 업데이트 해야한다. 
